...
Given a group $G$, a $G$-CW-Complex $X$ and $n \in \N$, is the n'th \ltwo-betti-number of $X$ equal to 0?
...
\proof[Proof of Theorem \ref{zero-divisor-problem:trivial_betti_numbers}]
	A finite type $G$-CW-complex can be described uniquely by its attaching maps which in turn can be described as a finite dimensional matrix with entries in $\Z[G]$. Also given such a matrix we can construct a unique $G$-CW-complex. The \ltwo-cohomology then corresponds to the kernel of that matrix.
