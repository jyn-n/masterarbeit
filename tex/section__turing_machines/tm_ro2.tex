\begin{tikzpicture}[shorten >=1pt,on grid,auto]
	...
	\node[state] (6) [right=5.5 of 4] {\textbf{3}};
	\node[state] (7) [below=7 of 4] {4};
	...
	\path[->]
		...
		(2) edge [ left ]
			...
			(3)
		(3) edge [ left ]
			node {$\begin{matrix}\ACC(M) \\ *\end{matrix}$}
			(8)
		...
		(7) edge [ bend left=10 , near start ]
			node {$(*,s),\begin{matrix}x \\ \underline{x}\end{matrix} \mapsto \begin{matrix}1 \\ 1\end{matrix},(x,s)$}
			(4)
		(7) edge [ bend left ]
