...
Without loss of generality we may assume that \IM{\left\vert A(M)~\cup~\{\DELIM\} \right\vert = 2^k} for some \IM{k \in \N}.
...
To see this, realize that every element of $\Z_2^l \setminus 0$ has characteristic 2 and thus any mapping \IM{x \mapsto y} for \IM{x,y \in \Z_2^l \setminus 0} can be extended to some (non-unique) automorphism $\phi_{x,y}$.
